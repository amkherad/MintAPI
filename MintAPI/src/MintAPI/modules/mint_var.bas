Attribute VB_Name = "mint_var"
'@PROJECT_LICENSE

Option Explicit
Option Base 0
Const CLASSID As String = "mint_var"

'DO NOT REMOVE ByVal VarPtr(xxx)'s

'=============================================
'=============================================
'=============================================
'<section API/Type Definitions>
'
'Public Declare Function API_CoTaskMemAlloc Lib "ole32" Alias "CoTaskMemAlloc" (ByVal cb As Long) As Long
'Public Declare Function API_CoTaskMemRealloc Lib "ole32" Alias "CoTaskMemRealloc" (pv As Any, ByVal cb As Long) As Long
'Public Declare Function API_CoTaskMemFree Lib "ole32" Alias "CoTaskMemFree" (pv As Any) As Long
...
Public Property Get CallerObject() As Object
    ...
    Call IUnknown_AddRef(CallerObject)
End Property
...
Attribute IUnknown_AddRef.VB_MemberFlags = "40"
...
Public Function IUnknown_AddRefPtr(ByVal TargetPtr As Long) As Long 'IUnknown::AddRef()
    If TargetPtr = vbNullPtr Then Exit Function
    
    Dim TIVBUnknown As IVBUnknown
    Call memcpy(TIVBUnknown, TargetPtr, VLEN_OBJECT)
    IUnknown_AddRefPtr = TIVBUnknown.AddRef
    Call memzero(TIVBUnknown, VLEN_OBJECT)
End Function
Public Function IUnknown_ReleasePtr(ByVal TargetPtr As Long) As Long 'IUnknown::Release()
    If TargetPtr = vbNullPtr Then Exit Function
    
    Dim TIVBUnknown As IVBUnknown
    Call memcpy(TIVBUnknown, TargetPtr, VLEN_OBJECT)
    IUnknown_ReleasePtr = TIVBUnknown.Release
    Call memzero(TIVBUnknown, VLEN_OBJECT)
End Function
Public Function IUnknown_QueryInterfacePtr(ByVal TargetPtr As Long, ByVal Guid As Guid, ByRef RetPtr As Long, ByRef RetObj As Object, Optional ByVal AutoAddRef As Boolean = False, Optional ByVal ThrowOnError As Boolean = True) As Long 'IUnknown::QueryInterface()
    If TargetPtr = vbNullPtr Then Exit Function
    If Guid Is Nothing Then throw Exps.ArgumentNullException
    Dim TIVBUnknown As IVBUnknown, RetRef As Long
    Call memcpy(TIVBUnknown, TargetPtr, VLEN_OBJECT)
    IUnknown_QueryInterfacePtr = TIVBUnknown.QueryInterface(Guid.ToStdGuid, RetRef)
    If ThrowOnError Then _
        If IUnknown_QueryInterfacePtr <> S_OK Then throw Exps.InvalidCastException("Unable To Cast To Specified Interface.")
    If AutoAddRef Then Call TIVBUnknown.AddRef
    Call memzero(TIVBUnknown, VLEN_OBJECT)
    'ObjectPtr(RetObj) = RetRef
    RetPtr = RetRef
...
Public Function GetArrayPointer(ByRef Arr As Variant, Optional ByVal throwOnNull As Boolean = False) As Long 'by Kelly Ethridge added on 7/23/2013
    ...
    Select Case VT And BYREF_ARRAY
        ...
        Case BYREF_ARRAY
            GetArrayPointer = MemLong(MemLong(VarPtr(Arr) + VARIANT_DATA_OFFSET))
        ...
        Case vbArray
            GetArrayPointer = MemLong(VarPtr(Arr) + VARIANT_DATA_OFFSET)
        Case vbEmpty, vbNull
            throw Exps.ArgumentNullException(Mtr("Array is null."))
        Case Else
            throw Exps.ArrayExpectedException("Arr")
    End Select
    ...
    Select Case VT And VT_TypeMask
        Case VBObject, vbUserDefinedType
            If MemLong(GetArrayPointer + SAFEARRAY_PVDATA_OFFSET) = vbNullPtr Then _
                GetArrayPointer = vbNullPtr
    End Select
    
    If throwOnNull Then
        If GetArrayPointer = vbNullPtr Then
            throw Exps.ArgumentNullException(Mtr("Null Array."))
        End If
    End If
End Function
...
Public Function EnsureArrayRange1D(ByVal pSafeArray As Long, ByVal Index As Long, ByRef Count As Long, ByRef outException As Exception) As Boolean
...
If Not IsArray(TargetArray) Then throw Exps.ArrayExpectedException("TargetArray")
    Dim ArrPtr As Long
    ArrPtr = GetArrayPointer(TargetArray)
