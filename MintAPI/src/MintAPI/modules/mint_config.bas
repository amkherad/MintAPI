...
'    If Not File.Exists(ConfigPath) Then _
...
'    If streaming_file Is Nothing Then   'throw Exps.InvalidStatusException("BeginConfigEditing() already called.")
'        Dim OpenConfigFile_retVal As New File
'        Set OpenConfigFile_retVal = File(ConfigPath)
...
'    If Not streaming_file Is Nothing Then  'throw Exps.InvalidStatusException("BeginConfigEditing() must be called before.")
...
'    If Not File.IsOpen Then throw Exps.InvalidStatusException("File not opened.")
...
'    Call f.WriteData(Instance.Version)
'
