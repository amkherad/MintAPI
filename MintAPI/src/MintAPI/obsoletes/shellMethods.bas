...
Public Function LoadIcon(Path As String) As Long
    If Dir(Path) = "" Then throw Exps.FileNotFoundException
    LoadIcon = API_shellMethods_LoadCursorFromFile(Path)
    If LoadIcon = 0 Then throw Exps.SystemCallFailureException
End Function
Public Function CreateTrayIcon(hWnd As Long, hIcon As Long, ToolTip As String, uID As Long, CallbackMessage As Long) As Long
    If Len(ToolTip) > NOTIFYICONDATATOOLTIPSIZE Then throw Exps.StringLengthOverflowException
...
Public Function Dialogs_BrowseFile( _
    ...
    If DialogType = OpenDialog Then
        If API_shellMethods_GetOpenFileName(f) = 0 Then throw Exps.OperationCanceledException
    Else
        If API_shellMethods_GetSaveFileName(f) = 0 Then throw Exps.OperationCanceledException
    End If
...
Public Function Dialogs_Browse(Optional ByVal OwnerForm As Long = 0, Optional ByVal Title As String = "", _
    ...
    Else
        throw Exps.OperationCanceledException
    End If
End Function
